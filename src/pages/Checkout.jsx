...
const Children = () => {
    ...
    const handleKindergartenSelect = (childId, kgId) => {
        setEnrollSelections(prev => ({ ...prev, [childId]: kgId }));
    };

    const handleEnroll = (child) => {
        const selectedKgId = enrollSelections[child.childId];
        const kg = kindergartens.find(k => k.kindergartenId === selectedKgId);
        if (!kg) return;
                ...
                <div className="cards-section">
                    {children.filter(c => !c.kindergartenId).map(child => (
                        <div key={child.childId} className="child-card">
                            ...
                            <select onChange={(e) => handleKindergartenSelect(child.childId, e.target.value)} value={enrollSelections[child.childId] || ''}>
                            ...
                            <button
                                ...
                                onClick={() => handleEnroll(child)}
                                disabled={!enrollSelections[child.childId]}
                            >
                                {t('children.proceedToPayment')}
                            </button>
                        </div>
                    ))}
                </div>
